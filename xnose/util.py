"""Utility functions and classes used by xnose internally.
"""
import inspect
import itertools
import logging
...
import types
import unittest
from xnose.pyversion import ClassType, TypeType, isgenerator, ismethod


log = logging.getLogger('xnose')
...
try:
    set()
    set = set # make from xnose.util import set happy
except NameError:
    try:
...
def ispackage(path):
    ...
    False
    >>> ispackage('xnose/plugins')
    True
    >>> ispackage('xnose/loader.py')
    False
    """
    if os.path.isdir(path):
...
def getpackage(filename):
    ...
    >>> getpackage('xnose')
    'xnose'
    >>> getpackage('xnose/plugins')
    'xnose.plugins'

    And __init__ files stuck onto directories

    >>> getpackage('xnose/plugins/__init__.py')
    'xnose.plugins'

    Absolute paths also work.

    >>> path = os.path.abspath(os.path.join('xnose', 'plugins'))
    >>> getpackage(path)
    'xnose.plugins'
    """
...
def resolve_name(name, module=None):
    ...
    >>> resolve_name('xnose.util') #doctest: +ELLIPSIS
    <module 'xnose.util' from...>
    >>> resolve_name('xnose.util.resolve_name') #doctest: +ELLIPSIS
...
def split_test_name(test):
    ...
    can alter xnose.util.split_test_re.
    """
...
def regex_last_key(regex):
    ...
    >>> from xnose.pyversion import sort_list
    >>> c = Config()
    >>> regex = c.testMatch
...
def transplant_func(func, module):
    ...
    """
    from xnose.tools import make_decorator
    if isgenerator(func):
        def newfunc(*arg, **kw):
