...
xnose's test loader implements the same basic functionality as its
...
from xnose.pyversion import unbound_method, ismethod
from xnose.case import FunctionTestCase, MethodTestCase
from xnose.failure import Failure
from xnose.config import Config
from xnose.importer import Importer, add_path, remove_path
from xnose.selector import defaultSelector, TestAddress
from xnose.util import func_lineno, getpackage, isclass, isgenerator, \
    ispackage, regex_last_key, resolve_name, src, transplant_func, \
    transplant_class, test_address
from xnose.suite import ContextSuiteFactory, ContextList, LazySuite
from xnose.pyversion import sort_list, cmp_to_key
...
class TestLoader(unittest.TestLoader):
    ...
    * Support easy extensions of or changes to that behavior through plugins
    ...
    def __init__(self, config=None, importer=None, workingDir=None,
        ...
        * config: provide a `xnose.config.Config`_ or other config class
          instance; if not provided a `xnose.config.Config`_ with
          default values is used.
        * importer: provide an importer instance that implements
          `importFromPath`. If not provided, a
          `xnose.importer.Importer`_ is used.
        ...
        * selector: a selector class or instance. If a class is
          ...
          current config. If not provided, a `xnose.selector.Selector`_
          is used.
        """
        if config is None:
            config = Config()
    ...
    def loadTestsFromName(self, name, module=None, discovered=False):
        ...
        within a module. See `xnose.util.split_test_name` for details on
        test name parsing.
        """
