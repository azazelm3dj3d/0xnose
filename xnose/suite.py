...
from xnose.util import isclass, resolve_name, try_run
import collections
        ...
        try:
            for test in self._tests:
                if result.shouldStop:
                    log.debug("stopping")
                    break
                # each xnose.case.Test will create its own result proxy
...
class ContextSuiteFactory(object):
    ...
    def __call__(self, tests, **kw):
        ...
        xnose.case.Test, be examined and the context of each found and a
...
class FinalizingSuiteWrapper(unittest.TestSuite):
    ...
    the standard test runner) where test running is not under xnose's
    control.
    """
...
class TestDir:
    def __init__(*arg, **kw):
        raise NotImplementedError(
            "TestDir is not usable with xnose 0.10. The class is present "
            "in xnose.suite for backwards compatibility purposes but it "
            "may not be used.")
...
class TestModule:
    def __init__(*arg, **kw):
        raise NotImplementedError(
            "TestModule is not usable with xnose 0.10. The class is present "
            "in xnose.suite for backwards compatibility purposes but it "
            "may not be used.")
