...
tests in all python modules. Normal xnose behavior is to look for tests only in
...
.. warning ::
   ...
   from what xnose normally considers non-test modules, such as
   the :doc:`doctest plugin <doctests>`. This is because any given
   object in a module can't be loaded both by a plugin and the normal xnose
   :class:`test loader <xnose.loader.TestLoader>`. Also, if you have functions
   or classes in non-test modules that look like tests but aren't, you will
   likely see errors as xnose attempts to run them as tests.
