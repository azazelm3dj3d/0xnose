...
class PluginTester(object):
    """A mixin for testing xnose plugins in their runtime environment.
    ...
    - activate
      ...
      - the argument to send xnosetests to activate the plugin
    ...
    - args
      ...
      - a list of arguments to add to the xnosetests command, in addition to
        the activate argument

    - env

      - optional dict of environment variables to send xnosetests
    ...
    def _execPlugin(self):
        ...
        self.xnose = TestProgram(argv=self.argv, config=conf, suite=suite,
                                exit=False)
        self.output = AccessDecorator(stream)

    def setUp(self):
        """runs xnosetests with the specified test suite, all plugins
        activated.
        """
        self.argv = ['xnosetests', self.activate]
        if self.args:
...
def munge_nose_output_for_doctest(out):
    """Modify xnose output to make it easy to use in doctests."""
...
def run(*arg, **kw):
    """
    Specialized version of xnose.run for use inside of doctests that
    test test runs.
    ...
    tests xnose (or unittest) test result output.

    Note: do not use doctest: +ELLIPSIS when testing xnose output,
    since ellipses ("test_foo ... ok") in your expected test runner
    output may match multiple lines of output, causing spurious test
    passes!
    """
    from xnose import run
    from xnose.config import Config
    from xnose.plugins.manager import PluginManager
    ...
    if kw.pop('buffer_all', False):
        ...
        restore = False
        warn("The behavior of xnose.plugins.plugintest.run() will change in "
             "the next release of xnose. The current behavior does not "
