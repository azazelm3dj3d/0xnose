...
the context. These fixtures will then execute in the primary xnose process, and
...
xnose process. There, any progress output is printed (dots!), and the
...
class MultiProcessTestRunner(TextTestRunner):
    ...
    def collect(self, test, testQueue, tasks, to_teardown, result):
        ...
        for case in self.nextBatch(test):
            ...
            if (isinstance(case, xnose.case.Test) and
    ...
    def addtask(testQueue,tasks,case):
        arg = None
        if isinstance(case,xnose.case.Test) and hasattr(case.test,'arg'):
    ...
    def nextBatch(self, test):
        ...
        if ((isinstance(test, ContextSuite)
            ...
            # special case: when run like xnosetests path/to/module.py
    ...
    def run(self, result):
        ...
        try:
            for test in self._tests:
                if (isinstance(test,xnose.case.Test)
