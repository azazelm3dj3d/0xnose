...
The plugin managers provided with xnose are:
...
:class:`BuiltinPluginManager`
    This manager loads plugins referenced in ``xnose.plugins.builtin``.
...
instantiating the :class:`xnose.config.Config` instance that you pass to
...
class EntryPointPluginManager(PluginManager):
    """Plugin manager that loads plugins from the `xnose.plugins` and
    `xnose.plugins.0.10` entry points.
    """
    entry_points = (('xnose.plugins.0.10', None),
                    ('xnose.plugins', ZeroNinePlugin))

    def loadPlugins(self):
        """Load plugins by iterating the `xnose.plugins` entry point.
        """
...
class BuiltinPluginManager(PluginManager):
    ...
    `xnose.plugins.builtin`.
    """
    def loadPlugins(self):
        """Load plugins in xnose.plugins.builtin
        """
        from xnose.plugins import builtin
