"""
xnosetests setuptools command
----------------------------

The easiest way to run tests with xnose is to use the `xnosetests` setuptools
command::

  python setup.py xnosetests

This command has one *major* benefit over the standard `test` command: *all
xnose plugins are supported*.

To configure the `xnosetests` command, add a [xnosetests] section to your
setup.cfg. The [xnosetests] section can contain any command line arguments that
xnosetests supports. The differences between issuing an option on the command
...
Here's an example [xnosetests] setup.cfg section::
...
If you commonly run xnosetests with a large number of options, using
the xnosetests setuptools command and configuring with setup.cfg can
...
files, and the xnosetests script will also load config files.)
...
can be more convenient than building by hand and running the xnosetests
script.
...
without having to install xnose themselves, add xnose to the setup_requires
section of your setup()::

  setup(
      # ...
      setup_requires=['xnose>=1.0']
      )

This will direct setuptools to download and activate xnose during the setup
process, making the ``xnosetests`` command available.

"""
try:
    from setuptools import Command
except ImportError:
    Command = xnosetests = None
else:
    from xnose.config import Config, option_blacklist, user_config_files, \
        flag, _bool
    from xnose.core import TestProgram
    from xnose.plugins import DefaultPluginManager
    ...
    class xnosetests(Command):
        description = "Run unit tests using xnosetests"
        ...
        def run(self):
            ...
            run xnose.main with a reconstructed argument list"""
            ...
            argv = ['xnosetests', '--where', ei_cmd.egg_base] 
