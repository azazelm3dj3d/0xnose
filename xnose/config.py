...
from xnose.util import absdir, tolist
...
class Config(object):
    ...
    Instances of Config are used throughout xnose to configure
    ...
    def configure(self, argv=None, doc=None):
        """Configure the xnose running environment. Execute configure before
        collecting tests with xnose.TestCollector to enable output capture and
        other features.
        """
        env = self.env
        if argv is None:
    ...
    def configureLogging(self):
        """Configure logging for xnose, or optionally other packages. Any logger
        name may be set with the debug option, and that logger will be set to
        debug level and be assigned the same handler as the xnose loggers, unless
        ...
        logger = logging.getLogger('xnose')
        logger.propagate = 0
        ...
        if self.debug:
            ...
            for logger_name in debug_loggers:
                ...
                if not l.handlers and not logger_name.startswith('xnose'):
                    l.addHandler(handler)

    def configureWhere(self, where):
        """Configure the working directory or directories for the test run.
        """
        from xnose.importer import add_path
        self.workingDir = None
        where = tolist(where)
        warned = False
        for path in where:
    ...
    def getParser(self, doc=None):
        ...
        parser.add_option(
            ...
            help="Output xnose version and exit")
        parser.add_option(
        ...
        parser.add_option(
            ...
            "Available debug loggers: xnose, xnose.importer, "
            "xnose.inspector, xnose.plugins, xnose.result and "
            "xnose.selector. Separate multiple names with a comma.")
        parser.add_option(
        ...
        parser.add_option(
            ...
            help="xnose's importer will normally evict a package from sys."
        ...
        parser.add_option(
            ...
            help="Prevent xnose from byte-compiling the source into .pyc files "
            "while xnose is scanning for and running tests.")
