...
Using the `xnose.plugins.multiprocess` plugin, you can parallelize a
...
the context. Fixtures for contexts so marked will execute in the primary xnose
...
The module with shared fixtures passes.
    ...
    >>> run(argv=['xnosetests', '-v', test_shared]) #doctest: +REPORT_NDIFF
    setup called
...
As does the module with no fixture annotations.
    ...
    >>> run(argv=['xnosetests', '-v', test_not_shared]) #doctest: +REPORT_NDIFF
    setup called
...
And the module that marks its fixtures as re-entrant.
    ...
    >>> run(argv=['xnosetests', '-v', test_can_split]) #doctest: +REPORT_NDIFF
    setup called
...
Then we can run the tests again with the multiprocess plugin active.
    ...
    >>> run(argv=['xnosetests', '-v', '--processes=2', test_shared],
...
Then we can run again and see the failures.
    ...
    >>> run(argv=['xnosetests', '-v', '--processes=2', test_can_split],
    ...     plugins=[MultiProcess()]) #doctest: +ELLIPSIS
    setup called
    teardown called
    ...
...
* More tests may be found
  ...
  processes load tests in *directed* mode -- the same way that xnose loads
  tests when you explicitly name a module -- rather than in *discovered* mode,
  the mode xnose uses when looking for tests in a directory.
...
* Plugin interaction warning
  ...
  wrap or gain control of the test-running process. Examples from xnose's 
...
* Python 2.6 warning
  ...
  This is unlikely to impact you unless you are writing tests for xnose itself,
  but be aware that under python 2.6, the multiprocess plugin is not
  re-entrant. For example, when running xnose with the plugin active, you can't
  use subprocess to launch another copy of xnose that also uses the
