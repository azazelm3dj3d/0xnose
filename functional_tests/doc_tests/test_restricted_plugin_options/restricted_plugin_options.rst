...
xnose is not able to use all available plugins. In those cases, a
`xnose.plugins.manager.RestrictedPluginManager` is used to exclude plugins that
implement API methods that xnose is unable to call.
...
method.
    ...
    >>> from xnose.plugins.manager import RestrictedPluginManager
...
plugins implementing `startTest`, an exception is raised and xnose exits.
...
either as an option defined by xnose, or by an active or excluded plugin, an
error is raised.
