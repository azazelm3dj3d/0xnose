...
from xnose.plugins.builtin import FailureDetail, Capture, Doctest
...
class IncludeUnderscoreFilesPlugin(Plugin):
    ...
    # Note that this is purely for purposes of testing xnose itself, and is
    # not intended to be a useful plugin.  In particular, the rules it
    # applies for _*.py files differ from the xnose defaults (e.g. the
...
class TestIncludeUnderscoreFiles(PluginTester, unittest.TestCase):
    ...
    def test_assert_info_in_output(self):
        ...
        # means that can't be done in xnose 0.10.
